...
class PineconeDataStore(VectorDataStore):
    def __init__(self):
    ...
    async def _query(
        self,
        queries: List[QuerySemanticWithEmbedding],
        ...
        async def _single_query(
            query: QuerySemanticWithEmbedding,
        ) -> QuerySemanticResult:
            ...
            return QuerySemanticResult(query=query.query, results=query_results)

        # Use asyncio.gather to run multiple _single_query coroutines concurrently and collect their results
        results: List[QuerySemanticResult] = await asyncio.gather(
